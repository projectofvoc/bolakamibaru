...
async function fetchLiveScores(): Promise<LiveMatch[]> {
...
export function useLiveScores(): UseLiveScoresResult {
  const { data, isLoading, error, refetch } = useQuery({
    ...
    staleTime: 30 * 1000, // 30 seconds - live data needs to be fresh
    refetchInterval: 60 * 1000, // Auto refetch every 60 seconds
    retry: 2,
  });

  // Return type tetap identik dengan versi sebelumnya
  return { 
    matches: data ?? [], 
    isLoading, 
