...
const Berita: React.FC = () => {
  ...
  const isIndonesianArticle = (article: any) => {
    ...
    return article.category === "Liga 1" || indonesianClubs.some((club) => title.includes(club)) || clubMatch;
  };
  ...
  const displayMatches =
    liveMatches.length > 0
      ? liveMatches
      : (liveMatchesData || [])
          .filter((m: any) => m.status === "ft" || m.status === "post" || m.status === "scheduled")
          .slice(0, 5);
  ...
  const pageTitle = currentFilter ? `${currentFilter.name[language]} - Berita BOLAKAMI` : "Berita - BOLAKAMI";
  ...
  return (
    <div className="min-h-screen flex flex-col bg-background">
      <Helmet>
        ...
        <meta property="og:url" content={`https://bolakami.com/berita${filter ? `/${filter}` : ""}`} />
        <meta property="og:type" content="website" />
        <meta name="twitter:card" content="summary_large_image" />
        <meta name="twitter:title" content={pageTitle} />
        <meta name="twitter:description" content={pageDesc} />
        <link rel="canonical" href={`https://bolakami.com/berita${filter ? `/${filter}` : ""}`} />
      </Helmet>
      <SidebarBanners />
      <Header />
      ...
      <main className="flex-1">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span>
                            {regionFeatured.published_at
                              ? new Date(regionFeatured.published_at).toLocaleDateString(
                        ...
                        displayMatches.map((match: any) => (
                          ...
                          >
                            ...
                            <div className="flex flex-col gap-0.5 min-w-0">
                              <span
                                className={`text-xs truncate ${match.homeScore !== undefined && match.homeScore > (match.awayScore || 0) ? "text-foreground font-semibold" : "text-muted-foreground"}`}
                              >
                                {match.homeTeam}
                              </span>
                              <span
                                className={`text-xs truncate ${match.awayScore !== undefined && match.awayScore > (match.homeScore || 0) ? "text-foreground font-semibold" : "text-muted-foreground"}`}
                              >
                                {match.awayTeam}
                              </span>
                            </div>
                            ...
                            <div className="flex flex-col gap-0.5 text-center">
                              <span
                                className={`text-xs font-bold ${match.homeScore !== undefined && match.homeScore > (match.awayScore || 0) ? "text-foreground" : "text-muted-foreground"}`}
                              >
                                {match.homeScore ?? "-"}
                              </span>
                              <span
                                className={`text-xs font-bold ${match.awayScore !== undefined && match.awayScore > (match.homeScore || 0) ? "text-foreground" : "text-muted-foreground"}`}
                              >
                                {match.awayScore ?? "-"}
                              </span>
                            </div>
            ...
            <div className="container mx-auto px-4">
              ...
              >
                ...
                <h1 className="text-3xl md:text-4xl font-bold text-foreground">
                  {currentFilter ? currentFilter.name[language] : language === "id" ? "Berita" : "News"}
                </h1>
              </motion.div>
        ...
        <section className="py-12 bg-card/50">
          <div className="container mx-auto px-4">
            ...
            <div
              className={`grid grid-cols-1 sm:grid-cols-2 ${isTrending ? "lg:grid-cols-5" : "lg:grid-cols-4"} gap-6`}
            >
