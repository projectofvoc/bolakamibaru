...
import { Trophy, ChevronRight, CheckCircle, Bookmark } from 'lucide-react';
            ...
            <div className="container mx-auto px-4">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {filteredMatches.slice(0, 4).map((match, idx) => (
                  ...
                  >
                    <span className="text-xs text-muted-foreground">{match.dateLabel[language]}, {match.time}</span>
                    <p className="text-sm font-semibold text-foreground mt-2">{match.homeTeam} vs {match.awayTeam}</p>
                  </motion.div>
                ))}
              </div>
            </div>
