...
import { Plus, Trash2, Edit2, Upload, LogOut, Video, Image as ImageIcon, GripVertical, X, Save } from 'lucide-react';
      ...
      <header className="sticky top-0 z-50 border-b border-border bg-card/95 backdrop-blur">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <span className="text-xl font-bold text-primary">CMS</span>
          </div>
          <div className="flex items-center gap-4">
        ...
        {momentsLoading ? (
          ...
          <Card className="text-center py-12">
            <CardContent>
              ...
              <p className="text-muted-foreground mb-4">Tambah momen terbaik pertama Anda</p>
          ...
          <Card>
            <CardHeader>
              <CardTitle>Daftar Momen ({moments.length})</CardTitle>
              <p className="text-sm text-muted-foreground">Drag untuk mengubah urutan</p>
            </CardHeader>
            <CardContent>
              <Reorder.Group axis="y" values={moments} onReorder={handleReorder} className="space-y-3">
                {moments.map((moment) => (
                  <Reorder.Item
                    key={moment.id}
                    value={moment}
                    className="flex items-center gap-4 p-4 bg-muted rounded-lg cursor-grab active:cursor-grabbing"
                  >
                    <GripVertical className="w-5 h-5 text-muted-foreground flex-shrink-0" />
                    
                    {/* Thumbnail */}
                    <div className="relative w-16 h-20 flex-shrink-0 rounded-lg overflow-hidden bg-card">
                      <img 
                    ...
                    <div className="flex-1 min-w-0">
                      <h4 className="font-medium text-foreground truncate">{moment.title_id}</h4>
                      <p className="text-sm text-muted-foreground truncate">{moment.title_en}</p>
                      <div className="flex items-center gap-2 mt-1">
                        {moment.video_url ? (
                          <span className="text-xs bg-primary/20 text-primary px-2 py-0.5 rounded-full">Video</span>
                        ) : (
                          <span className="text-xs bg-secondary text-secondary-foreground px-2 py-0.5 rounded-full">Gambar</span>
                        )}
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-3 flex-shrink-0">
                      <div className="flex items-center gap-2">
                        <Label htmlFor={`active-${moment.id}`} className="text-xs text-muted-foreground">
